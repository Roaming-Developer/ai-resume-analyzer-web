    ...
    <div id="export-content" style="font-family: Arial, sans-serif; padding: 20px; max-width: 800px; margin: 0 auto;">
      ...
      <div style="background: #f9fafb; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
        ...
        <div style="display: flex; align-items: center; gap: 20px;">
          <div style="font-size: 48px; font-weight: bold; color: ${getScoreColor(
            result.score
          )};">
            ${result.score}%
          </div>
          <div style="flex: 1; background: #e5e7eb; height: 20px; border-radius: 10px; overflow: hidden;">
            <div style="background: ${getScoreColor(
              result.score
            )}; height: 100%; width: ${result.score}%;"></div>
          </div>
        </div>
      </div>
      ...
      ${
        ...
        <div style="margin-bottom: 30px;">
          <h2 style="color: #1f2937; margin-bottom: 15px; font-size: 18px;">Matched Keywords (${
      ...
      ${
        result.missing_keywords.length > 0
          ? `
        <div style="margin-bottom: 30px;">
          <h2 style="color: #1f2937; margin-bottom: 15px; font-size: 18px;">Missing Keywords (${
      ...
      <div style="text-align: center; margin-top: 40px; padding-top: 20px; border-top: 1px solid #e5e7eb; color: #6b7280; font-size: 12px;">
        ...
        <p style="margin-top: 8px; font-size: 11px;">Result ID: ${
          result.result_id
        }</p>
      </div>
    </div>
